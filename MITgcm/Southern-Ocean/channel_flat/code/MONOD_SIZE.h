C $Header: /u/gcmpack/MITgcm_contrib/darwin2/verification/monod_1d_carbon/code/MONOD_SIZE.h,v 1.1 2011/04/13 19:25:26 jahn Exp $
C $Name:  $
...
C remember to bring the fields in data.ptracers in the right order !
      ...
      PARAMETER (iPOSih   = iZoC   +nzmax)
      PARAMETER (iPICh    = iPOSih +1) 
      PARAMETER (iPOChp   = iPICh  +1) 
      PARAMETER (iPOChs   = iPOChp +1)
      PARAMETER (iPOCsp   = iPOChs +1)
      PARAMETER (iPOCss   = iPOCsp +1) 
      PARAMETER (iPOCflux = iPOCss +1)
      PARAMETER (iPOChd   = iPOCflux +1)
      PARAMETER (iPOCsd   = iPOChd +1)
      PARAMETER (iDUSTh   = iPOCsd +1)
      PARAMETER (iDUSTs   = iDUSTh +1)
#else
